import { getCaseFromDb } from "./src/lib/cases";
import { generateCaseXai } from "./src/lib/xai.server";
...
async function main() {
  ...
  try {
    ...
    if ('summary' in xaiResult) {
      console.log("Summary field exists.");
      try {
        const parsedSummary = JSON.parse(xaiResult.summary as string);
        const keys = [
        ...
        keys.forEach(key => {
          if (key in parsedSummary) {
            ...
            console.log(`Key "${key}" MISSSING in summary.`);
          }
        });
      } catch (e) {
